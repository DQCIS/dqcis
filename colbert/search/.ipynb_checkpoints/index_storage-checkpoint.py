...
from colbert.modeling.colbert import colbert_score, colbert_score_packed, colbert_score_reduce,colbert_score_for_coverage, colbert_score_packed_for_coverage, colbert_score_reduce_for_coverage
...
class IndexScorer(IndexLoader, CandidateGeneration):
    ...
    def score_pids(self, config, Q, pids, centroid_scores):
        ...
        return colbert_score(Q, D_padded, D_mask, config, threshold, boost_factor=0.05), pids


    def score_pids_for_coverage(self, config, Q, pids, centroid_scores, threshold, boost_factor):
        """
            Always supply a flat list or tensor for `pids`.
        ...
        if self.use_gpu:
            ...
            for i in range(0, ceil(len(pids) / batch_size)):
                ...
                approx_scores_strided = StridedTensor(approx_scores_, pruned_codes_lengths, use_gpu=self.use_gpu)
                approx_scores_padded, approx_scores_mask = approx_scores_strided.as_padded_tensor()
                approx_scores_ = colbert_score_reduce_for_coverage(approx_scores_padded, approx_scores_mask, config, threshold, boost_factor)
                approx_scores.append(approx_scores_)
            approx_scores = torch.cat(approx_scores, dim=0)
            assert approx_scores.is_cuda, approx_scores.device
            if config.ndocs < len(approx_scores):
            ...
            approx_scores_strided = StridedTensor(approx_scores, codes_lengths, use_gpu=self.use_gpu)
            approx_scores_padded, approx_scores_mask = approx_scores_strided.as_padded_tensor()
            approx_scores = colbert_score_reduce_for_coverage(approx_scores_padded, approx_scores_mask, config, threshold, boost_factor)
            if config.ndocs // 4 < len(approx_scores):
                pids = pids[torch.topk(approx_scores, k=(config.ndocs // 4)).indices]
        else:
            pids = IndexScorer.filter_pids(
        ...
        if Q.size(0) == 1:
            return colbert_score_packed_for_coverage(Q, D_packed, D_mask, threshold, boost_factor, config), pids

        D_strided = StridedTensor(D_packed, D_mask, use_gpu=self.use_gpu)
        D_padded, D_lengths = D_strided.as_padded_tensor()

        return colbert_score_for_coverage(Q, D_padded, D_lengths, threshold, boost_factor, config), pids
    
    def coverage_rank_for_batch(self, config, Q, threshold, boost_factor, filter_fn=None, pids=None):
        with torch.inference_mode():
            if pids is None:
                pids, centroid_scores = self.retrieve(config, Q)
            else:
            ...
            scores, pids = self.score_pids_for_coverage(config, Q, pids, centroid_scores, threshold, boost_factor)
