...
class QueryTokenizer():
    ...
    def tensorize(self, batch_text, bsize=None, context=None, full_length_search=False):
        ...
        if full_length_search:
            ...
            un_truncated_ids = self.tok(batch_text, add_special_tokens=False, return_tensors='pt').to(DEVICE)['input_ids']
        ...
        obj = self.tok(batch_text, padding='max_length', truncation=True, return_tensors='pt', max_length=max_length).to(DEVICE)
        ...
        if context is not None:
            ...
            obj_2 = self.tok(context, padding='longest', truncation=True, return_tensors='pt', max_length=self.background_maxlen).to(DEVICE)
